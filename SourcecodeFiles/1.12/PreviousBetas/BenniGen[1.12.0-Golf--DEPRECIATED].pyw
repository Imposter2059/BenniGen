...
class NitroGenApp:
    def __init__(self, root):
        self.root = root
        self.root.title("BenniGen Golf v1.12")
    ...
    def show_primary_functions(self):
        if not self.primary_functions_running:
            ...
            if self.clutter_mode:
                self.check_all_codes_clutter(generated_codes)
            else:
                threading.Thread(target=self.check_all_codes_frame, args=(generated_codes,), daemon=True).start()
    ...
    def check_all_codes_frame(self, codes):
        results = {"valid_codes": []}
    ...
    def check_all_codes_clutter(self, codes):
        results = {"valid_codes": []}

        with concurrent.futures.ThreadPoolExecutor() as executor:
            try:
                future_to_code = {executor.submit(self.check_code, code, results): code for code in codes}
                ...
                print(f"Error in check_all_codes_clutter: {str(ex)}")
