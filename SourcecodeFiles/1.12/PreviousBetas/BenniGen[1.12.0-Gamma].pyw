...
class NitroGenApp:
    ...
    def create_widgets(self):
        ...
        tk.Button(self.root, text="Primary Functions", command=self.show_primary_functions, height=2, width=20).pack(pady=5)
        tk.Button(self.root, text="Render Speed", command=self.change_render_speed).pack(pady=5)
        tk.Button(self.root, text="Clutter Mode", command=self.toggle_clutter_mode).pack(pady=5)
